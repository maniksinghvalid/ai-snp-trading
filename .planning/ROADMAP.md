...
Plans:
- [ ] 01-01: MoomooGateway — persistent contexts, hard paper guard (PAPER_TRADING flag + SIMULATE acc-type assertion), run_in_executor wrappers, pre-flight connectivity check
- [ ] 01-02: StateStore — SQLite schema, atomic writes, startup load, and reconciliation loop skeleton
- [ ] 01-03: StrategyConfig (`rules.json`) loader/validator + StrategyCore ABC + TrendJoinLong + indicators — pure logic reads all params from config, unit tests, no I/O
